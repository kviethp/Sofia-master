...
<html lang="en">
  ...
  <body>
    ...
    <main>
      <section class="grid">
        <div class="panel">
          <div class="tiny">Runtime Mode</div>
          <div class="metric" id="runtime-mode">...</div>
        </div>
        <div class="panel">
          <div class="tiny">Queued Runs</div>
          <div class="metric" id="queued-count">...</div>
        </div>
        <div class="panel">
          <div class="tiny">Dead Letter</div>
          <div class="metric" id="dead-letter-count">...</div>
        </div>
        <div class="panel">
          <div class="tiny">Pending Approvals</div>
          <div class="metric" id="pending-approvals">...</div>
        </div>
      </section>
      ...
      <section class="grid">
        <div class="panel">
          ...
          <pre id="runtime-health">Loading...</pre>
        </div>
        <div class="panel">
          <h2>Policy Guardrails</h2>
          <pre id="policy-guardrails">Loading...</pre>
        </div>
        <div class="panel">
          <h2>Runtime Metrics</h2>
          <pre id="runtime-metrics">Loading...</pre>
        </div>
        <div class="panel">
          <h2>Task Status</h2>
      ...
      <section class="panel">
        ...
        <table id="dead-letter-table">
          <thead><tr><th>Run</th><th>Task</th><th>Step Count</th><th>Decision Count</th><th>Actions</th></tr></thead>
          <tbody></tbody>
        </table>
      </section>
      ...
      <section class="panel">
        ...
        <table id="approval-table">
          <thead><tr><th>Approval</th><th>Task</th><th>Status</th><th>Phase</th><th>Decision By</th><th>Actions</th></tr></thead>
          <tbody></tbody>
        </table>
      </section>
      ...
      <section class="panel">
        ...
        <table id="template-table">
          <thead><tr><th>Template</th><th>Risk</th><th>Workflow</th><th>Description</th></tr></thead>
          <tbody></tbody>
        </table>
      </section>
    ...
    <script>
      ...
      async function postJson(path, payload) {
        ...
        if (!response.ok) {
          throw new Error((body && body.message) || path + ' -> ' + response.status);
        }
        return body;
      }
      ...
      async function refresh() {
        try {
          const [runtime, metrics, deadLetterRuns, approvals, templates] = await Promise.all([
          ...
          fillTable('dead-letter-table', deadLetterRuns, (run) => \`
            <tr>
              ...
              <td>
                <div class="actions">
                  <button onclick="window.sofiaAdmin.replayRun('\${run.id}')">Replay</button>
                </div>
              </td>
            </tr>
          \`);
            ...
            <tr>
              ...
              <td>
                \${approval.status === 'pending' ? \`
                  <div class="actions">
                    <button onclick="window.sofiaAdmin.approveTask('\${approval.taskId}')">Approve</button>
                    <button class="danger" onclick="window.sofiaAdmin.rejectTask('\${approval.taskId}')">Reject</button>
                  </div>
